...
# 骆的 f1 计算函数
def evaluation_by_question(golden_list, predict_list):
    golden_set = set(golden_list)
    predict_set = set(predict_list)
    num_golden = len(golden_set)
    num_predict = len(predict_set)
    num_correct_predict = len(golden_set.intersection(predict_set))
    temp_precision = 0.0
    temp_recall = 0.0
    temp_f1 = 0.0
    if num_predict != 0:
        temp_precision = (num_correct_predict / num_predict)
    if num_golden != 0:
        temp_recall = (num_correct_predict / num_golden)
    if temp_precision + temp_recall != 0:
        temp_f1 = ((2 * temp_precision * temp_recall) / (temp_precision + temp_recall))
    precision = temp_precision
    recall = temp_recall
    f1 = temp_f1
    accuracy = (1.0 if temp_f1 == 1.0 else 0.0)
