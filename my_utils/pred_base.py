...
class PredBase:
    ...
    if (not os.path.exists(Config.cache_pred_info)) or (not os.path.exists(Config.cache_type_info)):
    ...
    def is_reverse_pair(cls, pred1:str, pred2:str) -> bool:
        ...
        if pred1.endswith("_Rev") and (not pred2.endswith("_Rev")) and pred1[:-4] == pred2:
            return True
        elif pred2.endswith("_Rev") and (not pred1.endswith("_Rev")) and pred2[:-4] == pred1:
            return True
        # 取逆关系进行判定
        else:
    ...
    def is_cvt_type(cls, tp:str) -> bool:
        ...
        if cls.type_info_dict[tp]["instance_count"] == 0:
            return False
        return (cls.type_info_dict[tp]["has_name_instance_count"] * 1.0 / cls.type_info_dict[tp]["instance_count"]) < 0.01
