...
class DataItem:
    ...
    def __init__(self, question_id:str, question:str, topic_ents:List[str], answers:List[str], lf=None, comp_type=None):
        self.id = question_id
...
# 从数据集中批量初始化数据项
def load_ds_items(filepath:str, sr_info_file:str=Config.ds_sr_all)->List[DataItem]:
    results = []
    if Config.ds_tag == "CWQ":
        ...
        for item in items:
            ...
            # topic_ents = ["ns:"+info["kb_id"] for info in sr_info[id]["entities"]]
            # answers = list(set(["ns:"+info["kb_id"] for info in sr_info[id]["answers"]]))
            topic_ents = []
            ...
            answers = ['ns:' + answer['answer_id']
                       if answer['answer_id'].startswith('m.') or answer['answer_id'].startswith('g.')
                       else answer['answer_id'] for answer in item['answers']]
            lf = item["sparql"]
