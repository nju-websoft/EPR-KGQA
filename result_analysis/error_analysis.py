# 错误分析辅助程序
# id，question，top1 ans cover，top10 ans cover，top10 ep
# ds_tag = "CWQ"
# import sys
...
for epinfo, ansinfo in tqdm(zip(test_ranked_eps, test_ranked_ans)):
  ...
  if ansinfo["hit"] == 0:
    top10_hit = False
    top1_hit = False
    top10_eps = get_instantiable_topk([item[0] for item in epinfo["sorted_candidates_with_logits"]], 10)
    top1_ep = top10_eps[:1]
    ...
    if len(gAns & set(top1_node_eps.keys())) > 0:
      top1_hit = True
    info = {"id":qid, "qtext":qtext, "top1_cover":top1_hit, "top10_cover":top10_hit, "top10_eps":top10_eps}
