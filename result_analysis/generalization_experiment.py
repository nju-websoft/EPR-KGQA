...
def split_cwq_test_by_iid_and_zero_shot():
    if os.path.exists(Config.ds_test_iid_idxs) and os.path.exists(Config.ds_test_zero_shot_idxs):
...
def split_test_dep_by_iid_and_zero_shot():
    ...
    test_dep_zero_shot = [item for idx, item in enumerate(test_dep) if idx in test_zero_shot_idxs]
...
def diff_topic_entities():
    test_items = load_ds_items("data/dataset/CWQ/ComplexWebQuestions_test.json",
    ...
    for idx, data_item in enumerate(test_items):
        ...
        if "State" in data_item.lf:
            print(1)
        if len(topic_ents_in_sparql - topic_ents_sr) > 0 or len(topic_ents_sr - topic_ents_in_sparql) > 0:
            print(1)
    print(1)
