...
class BiEncoderModule(torch.nn.Module):
    ...
    def forward(
        ...
        embedding_question = self.question_bert_layer(question_input_ids, question_attn_masks,
                                                      question_token_type_ids).pooler_output
        ...
        for i in range(0, relations_input_ids.shape[1]):
            ...
            embedding_relation = self.relation_bert_layer(relation_input_id, relations_attn_mask,
                                                          relations_token_type_id).pooler_output
    ...
    def calculate_loss(self, scores, golden_id):
        ...
        assert golden_id.shape[0] == scores.shape[0], print(
            'golden_id: {}, scores: {}'.format(golden_id.shape, scores.shape))
    ...
    def encode_question(self, question_token_ids, question_attn_masks, question_token_type_ids):
        ...
        """
        question_representation = self.question_bert_layer(question_token_ids, question_attn_masks,
                                                           question_token_type_ids).pooler_output  # (batch_size, 768)
    ...
    def encode_relation(self, relation_input_id, relations_attn_mask, relations_token_type_id):
        ...
        """
        relation_representation = self.relation_bert_layer(relation_input_id, relations_attn_mask,
                                                           relations_token_type_id).pooler_output  # (batch_size, 768)
