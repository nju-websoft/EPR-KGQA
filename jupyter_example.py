...
# 批量运行 topk ap 对 NSM 结果的影响（WebQSP）
ds_tag = "WebQSP"
num_step = 2
num_epoch = 50
import sys
...
ours_nsm_subg_file = (
    lambda tag: f"/home3/jmli/projects/IRQA/data/dataset/{ds_tag}_NSM/{tag}_simple.json"
)
...
for topk in range(20, 210, 20):
    ...
    # print(f"[test] ap top{topk}, ep top1: (Best F1) {best_f1_out[-1]}; (Best H1) {best_h1_out[-1]}")
    # print(f"[{ds_tag} Test] ap top{topk}, ep top1: (best_f1_model) {best_f1_out[-1]}")
