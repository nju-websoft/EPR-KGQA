...
For a quick start, we have uploaded critical models and data. You can download them in [this link](https://drive.google.com/drive/folders/1I_YSsbzG5JbGRb65DptVmXiOx7yPkAiV).
...
#### CWQ
For evidence pattern ranking, We have uploaded the trained model to [this link](https://drive.google.com/drive/folders/1x4wttiL7fYDy4zZj9NgiRhORksan14ye), place it to the corresponding path and then the step for training ranking model `sh -x evidence_pattern_retrieval/train_ep_ranking.sh CWQ`can be skipped. (And you can also comment out the code related to generating training data in the file `evidence_pattern_retrieval/generate_ep_ranking_data.py`, leaving only the code for generating prediction data.)
