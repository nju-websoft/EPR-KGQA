...
def gen_ent_rel_list(train_subgs, dev_subgs, test_subgs, entity_file, relation_file):
    ...
    logger.info(
        f"{len(all_ents)} unique entitied and {len(all_rels)} relations in top{Config.ep_topk} subgs"
    )
...
def prepare_nsm_input():
    ...
    gen_ent_rel_list(
        train_topk_subg, dev_topk_subg, test_topk_subg, entity_file, relation_file
    )
...
def check_relations():
    ...
    used_rels = set()
    # collect relations from induced subgraph (test ap=[20:200], ep=1)
    Config.ep_topk = 1
    for topk in range(20, 201, 20):
        Config.ap_topk = topk
        test_ents, test_rels = collect_covered_ent_rel(
...
def generate_test_file():
    ...
    test_topk_subg = read_json(Config.induced_subg_f("test"))
    ours_nsm_subg_file = (
        lambda tag: f"/home/jjyu/IRQA/data/dataset/{Config.ds_tag}_NSM/test_simple.json"
    )
