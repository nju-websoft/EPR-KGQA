...
from my_utils.ap_utils import filter_topk_preds
...
class AdjacentInfoPrepare:
  ...
  def load_global_cache(cls):
    ...
    cls.logger.debug(f"{len(processed_pred_set)} preds' adjacent info already in cache")
    
    for pred in PredBase.pred_info_dict:
      if pred not in processed_pred_set:
        preds_to_deal.append(pred)
    cls.logger.debug(f"{len(preds_to_deal)} preds' adjacent info need to be queried")
    ...
    run_multiprocess(mp_query_pred_conn, [Config.cache_pred_conn], preds_to_deal, Config.worker_num)
  ...
  @classmethod
  def get_ent_pred_adjacent_info(cls, ents:List[str], candi_preds:List[str]) -> Dict[str, Dict[str, List[str]]]:
    ans = dict()
    for ent in ents:
      preds = set(candi_preds)
      ans[ent] = {"fwd":[], "rev":[]}
      neighbor_preds = FreebaseODBC.query_neighbor_preds([ent])
      fwd_preds = list(set([pred for pred in neighbor_preds if not pred.endswith("_Rev")]) & preds)
      rev_preds = list(set([pred[:-4] for pred in neighbor_preds if pred.endswith("_Rev")]) & preds)
      ans[ent]["fwd"] = fwd_preds
      ans[ent]["rev"] = rev_preds
    return ans
  
  @classmethod
  def get_pred_pred_adjacent_info(cls, candi_preds:List[str]) -> Dict[str, Dict[str, List[str]]]:
    # 缓存信息使用时才载入内存
  ...
  @classmethod
  def parse_adjacent_info_from_trips(cls, trips:List[List[str]], dedup=True) -> Tuple[dict,dict]:
      ...
      for i in range(len(s_preds)):
        for j in range(i+1,len(s_preds)):
          ...
          if rel1 not in pred_pred_snippets:
            pred_pred_snippets[rel1] = {"S-S":set(), "S-O":set(), "O-O":set(), "O-S":set()}
          if rel2 not in pred_pred_snippets:
            pred_pred_snippets[rel2] = {"S-S":set(), "S-O":set(), "O-O":set(), "O-S":set()}
