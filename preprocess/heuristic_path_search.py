...
banned_base_preds = set(["ns:type.type.instance", 
                           "ns:type.object.type", 
    ...
    for item in tqdm(data_items):
        ...
        if item.id in cached_path:
            ...
            for topic_ent in topic_ents:
                paths = []
                forward_search(topic_ent, set(ans_ents), [topic_ent], paths, freebase, key_lexicals)
                search_succ = search_succ and len(paths) > 0
                results += paths
            append_jsonl({"id":item.id, "paths":results}, Config.cache_path)
        if not search_succ:
            fail_count += 1
    logger.info(f"[heuristic_search_path]({tag}) total:{len(data_items)}, skip count:{skip_count}, \
...
def forward_search(topic_ent:str, 
                   ans_ents:Set[str], 
                   cur_path:List[str], 
    ...
    # step3: obtain n-ary predstrs related to question, then do search on a new base path
        ...
        related_predstrs = filter_related_predstrs(nary_predstrs+binary_preds, key_lexical)
        for predstr in related_predstrs:
            new_path = deepcopy(cur_path)
            new_path.append(predstr)
            if satisfy_base_path_condition(new_path):
                forward_search(topic_ent, ans_ents, new_path, all_paths, freebase, key_lexical)
    ...
    for idx in range(len(preds))[1:]:
        if idx + 1 < len(preds):
            ...
            if PredBase.is_reverse_pair(first,second):
                return False
    # Rule3: 如果 path 最后一个谓词的 Range 是字面量，也不能作为 base path
    # TODO
    return True
...
def run(run_tag=Config.bulk_path_search):
    ...
    logger.info(f"{len(train_set)} items in train set; {len(dev_set)} items in dev set;")
