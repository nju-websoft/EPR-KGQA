...
from my_utils.io_utils import read_json, write_json, append_jsonl, calculate_PRF1, run_multiprocess
...
logger = Logger.get_logger("CG_ranker_td_gen")
...
def td_gen_by_ans_coverage(ds_item:DataItem, 
...
def mp_td_gen_by_ans_coverage(target_file, pid, queue):
  while True:
    ...
    if info == None:
      break
    ds_item = info["item"]
    ent_preds = info["ep_lp"]
    pred_preds = info["pp_lp"]
...
def collect_train_data(split_tag:str, lp_topk:int, use_multi_process:bool):
  print(f">>> collect train data for ep ranking ({split_tag})...")
  assert split_tag in set(["dev", "train"])  
  print("Reading dataset & retrieved lps")
  if split_tag == "dev":
    items = load_ds_items(Config.ds_dev)
  else:
    items = load_ds_items(Config.ds_train)
  retrieved_lps = filter_topk_lps(read_json(Config.retrieved_ap_f(split_tag)), lp_topk)
  ...
  logger.info(f"{len(processed_ids)} dev items already processed, {len(items)-len(processed_ids)} items left.")
  items_to_deal = []
  for item, lp_info in zip(items, retrieved_lps):
    assert item.id == lp_info["id"]
    if item.id not in processed_ids:
      ep_lps, pp_lps = get_retrieved_lps(item.topic_ents, lp_info)
      items_to_deal.append({"item":item, "ep_lp":ep_lps, "pp_lp":pp_lps})
  print(f"Generate training data, use_multi_process: {use_multi_process}")
  if use_multi_process:
    run_multiprocess(mp_td_gen_by_ans_coverage, [td_f], items_to_deal, Config.worker_num)
  else:
    ...
    for info in tqdm(items_to_deal):
      td_info = td_gen_by_ans_coverage(info["item"], info["ep_lp"], info["pp_lp"])
      ...
      if len(td_info["positive"]) == 0:
        fail_count += 1
      if count == 100:
        print(f"#item:{count}, #pos:{pos_count}, #neg:{neg_count}, #fail:{fail_count}")
        break
      ### [To Del]
